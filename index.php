 ...
 *
 * REVISI: Memperbaiki 'case submit_section' 
 * Mengubah 'ON DUPLICATE KEY UPDATE value = :value'
 * menjadi '... value = VALUES(value)' untuk memperbaiki error HY093.
 */
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        switch ($action) {
            ...
            case 'submit_section':
                ...
                if ($role === 'teknisi' || $role === 'manajer') {
                    if ($current_section_id === 1) $next_section_id = 2;
                    elseif ($current_section_id === 2) $next_section_id = 3;
                    elseif ($current_section_id === 3) $next_section_id = 4;
                } 
                
                if ($current_section_id === 4 || $current_section_id === 5) {
                ...
                if ($next_section_id) {
                    ...
                    $stmt_check = $pdo->prepare("
                        ...
                        WHERE section_id = ? AND is_active = 1 {$role_filter_sql} LIMIT 1
                    ");
                    $stmt_check->execute([$next_section_id]);
                    
                    if (!$stmt_check->fetch()) {
                        // Tidak ada pertanyaan di S4 (atau section selanjutnya), anggap selesai
                        $next_section_id = null; 
                    }
                }
...
$view_file = __DIR__ . "/views/{$current_step}.php";
...
if (file_exists($view_file)) {
    ...
    if (isset($error_message)) {
        echo '<div class="row justify-content-center"><div class="col-md-10 col-lg-8">';
